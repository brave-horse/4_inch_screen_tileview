...
static void wifi_app_task(void *priv)
{
...
#endif
    wifi_set_store_ssid_cnt(NETWORK_SSID_INFO_CNT);    // 设置最多保存多少组连接过的 WiFi 信息
    wifi_set_event_callback(wifi_event_callback);      // 注册 WiFi 事件回调, 连接/断开/DHCP 等事件会进这里
    wifi_on();                                         // 打开 WiFi 模块, 启动 WiFi 驱动和内部相关任务
    wifi_set_long_retry(4);                            // 设置长帧发送失败后的重试次数
    wifi_set_short_retry(7);                           // 设置短帧发送失败后的重试次数
    ...
    net_app_init();
    sys_timer_add_to_task("sys_timer", NULL, wifi_app_timer_func, 5 * 1000);//这里建立的任务
}
