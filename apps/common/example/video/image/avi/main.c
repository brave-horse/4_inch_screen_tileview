...
#define AVI_PATH 	"mnt/sdfile/EXT_RESERVED/uipackres/ui/"
...
static void dec_jpg_to_yuv(void *priv, char *buf, int buf_len)
{
    ...
    if (jpeg_decode_image_info(&info)) { //获取JPEG图片信息
        ...
        int ret = jpeg_decode_one_image(&req, JPEG_DEV_ID_0);//JPEG转YUV解码
        ...
        lcd_show_frame(avi->yuv_buf, info.width * info.height * 3 / 2, info.width, info.height);
        Calculation_frame();
    }
}
...
static void avi_audio_task_inti(void *priv)
{
    ...
    while (1) {
        ...
        if (avi->audio_run_num >= avi->audio_num) {
            ...
            free(avi->audio_buf);
            avi->audio_buf = NULL;
            os_sem_del(&avi->rd_sem, OS_DEL_ALWAYS);
...
static void avi_jpg_task_inti(void *priv)
{
    ...
    while (1) {
        ...
        if (r_jpg_len > 0) {
            ...
            avi->jpg_buf = NULL;
            free(avi->yuv_buf);
            os_sem_del(&avi->jpg_run, OS_DEL_ALWAYS);
            break;
        }
    }
}
