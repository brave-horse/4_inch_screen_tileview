...
#define DEMO_UI_LCD_PUTS(str) puts(str)
#else
#define DEMO_UI_LCD_PUTS(str) do { } while (0)
#endif
...
{
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_open_send_code_port enter\n");
    if (!lcd) {
    ...
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_open_send_code_port done\n");
    return 0;
}
...
{
    int ret;
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_send_init_code enter\n");
    if (!lcd) {
    ...
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_send_init_code done\n");
    return 0;
}
...
static void lcd_early_init_task(void *arg)
{
    ...
    printf("[demo_ui_boot] lcd_early_init_task enter, lcd_id=%d\n", lcd_id);
    if (!lcd || !bd_cfg) {
    ...
    if (lcd_open_send_code_port(lcd, bd_cfg)) {
        DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_open_send_code_port fail\n");
        return;
    }
    ...
    if (lcd->init) {
        DEMO_UI_LCD_PUTS("[demo_ui_boot] before lcd->init\n");
        lcd->init(bd_cfg);
        DEMO_UI_LCD_PUTS("[demo_ui_boot] after lcd->init\n");
    } else {
    ...
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd early init sem posted\n");
...
static int lcd_dev_init(const struct dev_node *node, void *pdata)
{
    ...
    int bd_num;
    struct lcd_dev_drive *p;
    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_dev_init enter\n");
    ...
    printf("[demo_ui_boot] lcd_dev_match logo=%s, id=%d\n", p ? p->logo : "NULL", p ? p->id : -1);
    u8 lcd_id = p->id;
    __this->lcd = p;
    __this->bd_cfg = lcd_bd_cfg_match((char *)NULL, lcd_pdata);
    printf("[demo_ui_boot] lcd_bd_cfg_match name=%s\n", __this->bd_cfg ? __this->bd_cfg->lcd_name : "NULL");
    __this->is_early_init = 1;

    int fork_ret = thread_fork("lcd_early_init_task", 30, 1024, 0, 0, lcd_early_init_task, (void *)&p->id);
    printf("[demo_ui_boot] lcd_early_init_task fork_ret=%d\n", fork_ret);
#endif

    DEMO_UI_LCD_PUTS("[demo_ui_boot] lcd_dev_init done\n");
    return 0;
}
