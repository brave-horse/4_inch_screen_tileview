...
static char lvgl_suspend_flag = 1; //1 = LVGL 暂停/未运行 ；0 = LVGL 正常运行
...
{
    lvgl_suspend_flag = 0;                  // LVGL任务进入运行状态
    int msg[8] = {0};                       // UI任务消息缓存
    int ret;                                // 消息等待结果
    lvgl_touch_timer_status = 0;            // 触摸轮询定时器关闭
    char lvgl_task_pend_timeout_refr_flag = 0; // 任务等待超时后的刷新标志
    u32 time_till_next;                     // 下一次LVGL定时器间隔
