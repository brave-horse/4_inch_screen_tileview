...
#define DEMO_UI_BOOT_PUTS(str) puts(str)
#else
#define DEMO_UI_BOOT_PUTS(str) do { } while (0)
#endif
...
{
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] lvgl_v8_main_task enter\n");
    lv_init();
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] lv_init done\n");
    lv_port_disp_init();
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] lv_port_disp_init done\n");
    lv_port_indev_init();
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] lv_port_indev_init done\n");
    lv_port_fs_init();
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] lv_port_fs_init done\n");

#ifdef CONFIG_DEMO_UI_PROJECT_ENABLE
    /* 注册 SD 资源包到 LVGL 的 sdfile 系统(设置 sdfile_num), 让杰理 JPEG 解码器
     * lv_sdfile_get_jpeg_info_and_malloc 能按 s:/ 路径查到包里的图。
     * 这步之前随 img.c 一起被删 → sdfile_num=0 → 主屏 1.jpg(JPEG) 读不出。
     * 注: PNG 走 fs_open 不依赖它, 只有 JPEG 需要。返回值=注册到的文件数(>0 即成功)。 */
    {
        extern uint16_t lv_load_pack_sdfile(const char *src);
        uint16_t _ui_pack_n = lv_load_pack_sdfile("mnt/sdfile/EXT_RESERVED/uipackres");
        printf("[demo_ui_boot] lv_load_pack_sdfile(uipackres) -> %u\n", _ui_pack_n);
    }
#endif
    ...
    lvgl_suspend_flag = 0;
    jl_gui_init();
    DEMO_UI_BOOT_PUTS("[demo_ui_boot] jl_gui_init done in lvgl task\n");
